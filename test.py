...
if __name__ == '__main__':
    ...
    targetPathTest = opt.Target_Path_Test
    ...
    print('--------------------------------------------------------------')
    if opt.CUDA_USE:
        myNet.load_state_dict(torch.load('./pretrained/model_best_.pth'))
    else:
        myNet.load_state_dict(torch.load('./pretrained/model_best_.pth', map_location=torch.device('cpu')))
    myNet.eval()
    ...
    with torch.no_grad():
        ...
        for index, (x, name) in enumerate(tqdm(testLoader, desc='Testing !!! ', file=sys.stdout), 0):
            ...
            input_test, pad_size = pad(input_test, factor=128)
